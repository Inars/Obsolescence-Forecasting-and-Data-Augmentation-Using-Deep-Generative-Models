...
def main():
    ...
    synthesizer.save(filepath='../models/generative/tvae_' + DATASET_NAME + '.pkl')
    history.to_csv('../models/generative/history/tvae_' + DATASET_NAME + '.csv', header=True, index=False)
    synthetic_data.to_csv('../data/generated/TVAE/' + DATASET_NAME + '.csv', header=True, index=False)
